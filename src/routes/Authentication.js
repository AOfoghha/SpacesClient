...
import { AsyncStorage, Text, Image, TextInput, TouchableOpacity, View, StyleSheet } from 'react-native';
...
export default class Authentication extends Component {
	...
	userLogin() {
		if (this.state.login && this.state.password) {
			...
			axios('http://spcs.me/neoapi/session', {
				...
				.then((response) => {
					this.onValueChange('sid', response.sid),
	...
	userCheck() {
		fetch('http://spcs.me/neoapi/session', {
			method: 'POST',
			headers: {
				'X-Proxy': 'spaces',
				'Cookie': 'json=1'
	...
	userLogOut() {
		fetch('http://spcs.me/neoapi/session?json=1', {
			method: 'POST',
			credentials: 'include',
			headers: {
		...
		return (
			<View style={styles.container}>
				...
				<View style={styles.signupSocialButtom}>
					<TouchableOpacity style={styles.vk} onPress={this.userLogin.bind(this)}>
					...
					<TouchableOpacity style={styles.ok} onPress={this.userLogin.bind(this)}>
					...
					<TouchableOpacity style={styles.mail} onPress={this.userLogin.bind(this)}>
					...
					<TouchableOpacity style={styles.fb} onPress={this.userLogin.bind(this)}>
						<Svg width="48" height="18" viewBox="0 0 1792 1792">
				...
				<View style={styles.signupTextSocial}>
					<TouchableOpacity onPress={this.signup} onPress={() => Actions.Restore()}>
						<Text style={styles.signupButton}>Забыли ник или пароль?</Text>
					</TouchableOpacity>
				</View>
				<View style={styles.signupTextCont}>
					<Text style={styles.signupText}>Нет учетной записи?</Text>
					<TouchableOpacity onPress={() => Actions.Registration()}><Text style={styles.signupButton}> Регистрация</Text></TouchableOpacity>
				</View>
			</View>
		);
	}
